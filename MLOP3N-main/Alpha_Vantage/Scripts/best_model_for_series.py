...
output = pd.DataFrame(
    columns=["Company Name", "Best Model", "Best Distribution", "Economic Significance"]
)
cnt = 0
for one in range(26, 41):
    ...
    for two in models:
        for three in dists:
            ...
            model_pred = pd.read_csv(
                path, names=["return_prediction", "volatility_prediction"], header=0
            )
